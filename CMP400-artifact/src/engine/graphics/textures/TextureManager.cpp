...
namespace JEngine
{
	...
	void TextureManager::LoadTexture(const std::string& id, const std::string& filepath)
	{
		...
		m_textures.insert(std::make_pair(id, std::make_unique<Texture>(m_device, filepath)));
	}
	...
	{
		m_textures.insert(std::make_pair("default", std::make_unique<Texture>(m_device, "media/textures/DefaultDiffuse.png")));
	}
